...
int main() {
    ...
    for (double h : step) {
        ...
        runge_kutta(x, y_rk, h, df, true);
        milne(x, y_m, h, df, true);
        
        std::cout << "step: " << h << std::endl;
        std::cout << "Runge-Kutta method absolute error in x = "
                  << x[idx_mid] << ": " << absolute_error(y_rk[idx_mid], y_correct[idx_mid]) << std::endl;
        std::cout << "Milne method relative error in x = "
                  << x[idx_mid] << ": " << relative_error(y_m[idx_mid], y_correct[idx_mid]) << std::endl;
        std::cout << "Runge-Kutta method absolute error in x = "
                  << x[idx_end] << ": " << absolute_error(y_rk[idx_end], y_correct[idx_end]) << std::endl;
        std::cout << "Milne method relative error in x = "
                  << x[idx_end] << ": " << relative_error(y_m[idx_end], y_correct[idx_end]) << std::endl;
        std::cout << std::endl;
    }
    ...
    std::cout << "Runge-Kutta method absolute error for f1 in x = "
                  << x[idx_end] << ": " << absolute_error(y1[idx_end], y1_correct[idx_end]) << std::endl;
    std::cout << "Runge-Kutta method relative error for f1 in x = "
                  << x[idx_end] << ": " << relative_error(y1[idx_end], y1_correct[idx_end]) << std::endl;
    std::cout << "Runge-Kutta method absolute error for f2 in x = "
                  << x[idx_end] << ": " << absolute_error(y2[idx_end], y2_correct[idx_end]) << std::endl;
    std::cout << "Runge-Kutta method relative error for f2 in x = "
                  << x[idx_end] << ": " << relative_error(y2[idx_end], y2_correct[idx_end]) << std::endl;
