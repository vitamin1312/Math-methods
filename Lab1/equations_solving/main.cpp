...
int main() {
    ...
    for (auto [a, b] : intervals1) {
        ...
        all_x = iterations((a+b)/2, a, b, 0.00001, &f1, &df1, &fi, &dfi); 
        N = all_x.size();
        x = all_x[N-1];
    ...
    vector<pair<double, double>> all_x_y = iterations_system(2.2, 0.4, 0.00001, &fi1, &fi2);
    N = all_x_y.size();
    x = all_x_y[N-1].first;
    ...
    all_x_y = iterations_system(-0.4, 2.2, 0.00001, &fi11, &fi22);
    N = all_x_y.size();
    x = all_x_y[N-1].first;
    y = all_x_y[N-1].second;
    cout << "x: " << x << "; y: " << y << "; number of iterations: " << N-1 << endl;
